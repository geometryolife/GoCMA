// 1. 切片是对数组一个连续片段的引用，它是一个容量可变的序列。
// 2. 切片类似于一个动态数组，通过指针引用底层数组。
// 3. 结构:
// 底层数组指针、大小、容量
// array, len, cap
package main
...
// 动态创建切片
// make 函数动态创建切片，在创建过程中指定切片的长度和容量
// make([]T, size, cap)
func dynamicSlice() {
	sli := make([]int, 2, 4)
	fmt.Println()
...
func appendSlice() {
	...
	// 容量足够的 sli1 直接将 append 添加的新元素覆盖到原有数组 arr1 中
	...
	// 容量不够的 sli2 进行了扩容操作，申请新的底层数组，不在原数组的基础上操作
	...
	// 片自身的容量已经饱和，此时进行 append 操作，同样会进行扩容，申请
	// 新的内存空间
...
ex len is 0
ex cap is 0
address of ex is 0x57fb60
ex2 value is [1 2 3]
ex2 len is 3
ex2 cap is 3

address of ex2 is 0xc0000181e0
address of &ex2[0] is 0xc0000181e0
address of &ex2[1] is 0xc0000181e8
address of &ex2[2] is 0xc0000181f0

sli1 pointer is 0xc00000c140, len is 2, cap is 4, value is [1 2]
sli2 pointer is 0xc00000c160, len is 2, cap is 2, value is [3 4]
sli3 pointer is 0xc00000c180, len is 2, cap is 2, value is [1 2]
newSli1 pointer is 0xc00000c200, len is 3, cap is 4, value is [1 2 5]
source arr1 become [1 2 5 4]
newSli2 pointer is 0xc00000c240, len is 3, cap is 4, value is [3 4 5]
source arr2 become [1 2 3 4]
newSli3 pointer is 0xc00000c280, len is 3, cap is 4, value is [1 2 5]
