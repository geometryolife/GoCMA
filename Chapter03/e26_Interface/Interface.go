...
// 可使用的方法而无需了解具体实现，实现方通过接口对外提供能
// 使用的特性。
// 3. 每个接口由一个或者多个方法组成。
// 4. 定义接口需要配合使用 type 和 interface 关键字。
// 5. 接口名的首字母和方法名的首字母皆大写时，表示该方法是公开的。
// 6. Golang 的接口具有嵌套特性，可以实现类似面向对象中的接口继承
// 特性，从而创造出新的接口。
// 7. Golang 中所有类型都可以实现接口，函数作为 Golang 中的一种类
// 型，也不例外。
// 8. 通过函数类型实现接口，可以将函数作为接口来使用，在运行时可
// 以通过替换具体的实现函数，实现类似多态的效果。
